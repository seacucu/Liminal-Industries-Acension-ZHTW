...
（選用）協同處理器藉由加快 CPU 的運轉頻率，讓系統更頻繁地從 <ItemLink id="pattern_provider" /> 送出材料批次，
好跟上那些處理速度很快的機器。舉例來說，一個被 <ItemLink id="molecular_assembler" /> 包圍的樣板供應器，
推料速度可以超過單一台組裝機的處理速度，於是材料批次就會分散給周圍的多台組裝機。

有些複雜的配方，中間好幾個步驟其實可以平行進行，例如做書櫃時同時做木材與書。
在合成狀態畫面上（右鍵點擊 CPU，或按[終端機](terminals.md)裡的錘子圖示），這些步驟都會顯示為「已排程」。
每多一個協同處理器，就能多讓一個步驟平行進行（也就會顯示為「合成中」）。
不過這一點通常沒那麼要緊，因為你為了推料速度而放的協同處理器數量，往往已經多過配方裡真正能平行的步驟數。
