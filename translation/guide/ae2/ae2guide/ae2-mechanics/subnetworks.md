...
「子網路」的定義相當寬鬆，大致可以說：任何用來輔助主網路、或負責某項小任務的[網路](../ae2-mechanics/me-network-connections.md)都算。
...
做子網路時最重要的一件事，是隨時掌握[網路連線](../ae2-mechanics/me-network-connections.md)的狀況。
很常見的情況是，有人把一堆介面與匯流排湊在一起就以為那是子網路，
結果所有裝置其實還透過各種整格裝置連在主網路上。

線纜的顏色和「能不能做出子網路」沒有關係，唯一的作用是不同顏色的線纜彼此不會連接。

舉幾個例子：

* 用一整個 AE2 網路取代垃圾桶或銷毀升級，讓它自己決定怎麼處理你的垃圾最有效率：
  依照供需與可用性，把物品聰明地送去堆肥箱陣列，或某個模組的回收機。
* 建立抽象層。把一套複雜合成作業的所有細節都關在子網路裡，這樣從主網路的角度看，
  整座工廠「看起來」就只是一台機器。
* 平行化。把一台慢機器換成十台一樣的慢機器。從主網路的角度看什麼都沒變，
  而且你連一條頻道都沒多用。
